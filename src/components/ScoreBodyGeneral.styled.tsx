...
export const ScoreDetailsArea = styled.div`
  /* padding-top: 3rem; */
  width: 27.25rem; // 기존 63.75rem (1020 px)
  height: 19rem;
  display: flex;
  flex-direction: column;
...
export const ScoreLabel = styled.div`
  ...
  p {
    font-size: 1.125rem; // 기존 20px -> 18px
    font-weight: bold;
    margin-right: 0.3rem;
  }
`;
...
export const ScoreBarContainer = styled.div`
  width: 20rem; // 기존 25rem (400px) -> 20rem (320px)
  height: 1.5rem; // 기존 28rem -> 24rem
  background: #d9d9d9;
  border-radius: 1.25rem; //20px
  overflow: hidden;
`;
...
export const ScoreValue = styled.div<{ width: number; color: string }>`
  // ScoreBar 너비 * 점수 * 0.01 (백분율) * 1/16 (px to rem)
  padding-left: ${({ width }) => width / 5 - 0.5}rem;
  text-align: left;
  font-size: 1rem;
  font-weight: bold;
